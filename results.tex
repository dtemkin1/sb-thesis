...
The benefits are not limited to fleet utilization. Across all tracked student groups, \gls{bra} reduces estimated mean travel times relative to the current routing baseline, as shown in \autoref{tab:travel_times}. For currently assigned bus riders, mean travel time falls from 43.08 minutes to 30.28 minutes, a reduction of approximately 29.7\%, while the maximum observed travel time falls from 133.38 minutes to 59.61 minutes. Vulnerable groups also see consistent improvements (\autoref{tab:travel_times}): mean travel time declines from 41.24 to 31.23 minutes for students from households where English is not the primary language, from 39.83 to 31.54 minutes for students from non-car-owning households, and from 36.68 to 25.34 minutes for students with disabilities. \autoref{fig:routes} shows the current and proposed route layouts side-by-side.
...
\begin{figure}[!htbp]
    ...
    \caption{Pareto fronts for \gls{bra} routing configuration with student data.}
    \label{fig:pareto}
\end{figure}
